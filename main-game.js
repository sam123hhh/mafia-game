...
let mgRevealCard = null; // {name, role} — يظهر للهوست بس، مع أنيميشن كشف الكرت
...
async function mgAddWarning(seatId){
  ...
  if(w>=3){
    ...
    if(seat.role){ mgRevealCard = { name: seat.name, role: seat.role }; }
    mgRender();
  }else{
...
async function mgEliminateVote(seatId){
  ...
  if(seat && seat.role){ mgRevealCard = { name: seat.name, role: seat.role }; }
  mgRender();
}
...
async function mgFinishNight(){
  ...
  if(round >= 2){
    ...
    if(assassinActed && a.killedSeatId){
      ...
      if(!saved && victimSeat){
        killedName = victimSeat.name;
        await update(ref(db,`mainGames/${mg.roomCode}/seats/${a.killedSeatId}`), { status:'removed' });
        if(victimSeat.role){ victimToReveal = { name: victimSeat.name, role: victimSeat.role }; }
      }
    }
  }
  ...
  if(victimToReveal){ mgRevealCard = victimToReveal; }
  mgRender();
}
...
async function mgHandleAction(act, val){
  ...
  if(act==='mg-close-reveal'){ mgRevealCard=null; mgRender(); return; }
...
function mgViewHostTable(){
  ...
  ${mgRevealCard? mgRevealModalHtml() : ''}
  ${timerFrozen? `
...
// كشف كرت اللاعب للهوست بس (بعد إقصاء تصويت، إنذارات، أو اغتيال) — بأنيميشن كشف مناسب للدور
function mgRevealModalHtml(){
  if(!mgRevealCard) return '';
  const meta = MG_ROLE_META[mgRevealCard.role] || MG_ROLE_META.citizen;
  return `
  <div class="modal-back">
    <div class="verdict-card">
      <p class="eyebrow" style="justify-content:center;">تم كشف الكرت</p>
      <div class="flip-card flipped" style="margin:10px auto 16px;">
      ...
      <div class="verdict-title" style="color:${meta.color};">${esc(mgRevealCard.name)}</div>
      <div class="verdict-sub">طلع من الطاولة</div>
      <button class="btn btn-primary" data-act="mg-close-reveal" style="margin-top:14px;">إغلاق</button>
    </div>
  </div>`;
}
...
function mgViewPlayerTableInner(){
  ...
  if(seat.role==='silencer' || seat.role==='assassin'){
    ...
    // مافيا التسكيت: تقدر تسكت أي حدا بدون استثناء، حتى المافيا الثانية وحتى نفسها.
    // مافيا الاغتيال: بتضل مستثنية عن استهداف فريقها متل ما هو معمول.
    const others = isSilencer
      ? Object.entries(mgRoom.seats).filter(([id,s])=> s.status==='active')
      : Object.entries(mgRoom.seats).filter(([id,s])=> s.status==='active' && !MG_MAFIA_ROLES.includes(s.role) && id!==seatId);
    ...
    <div class="card">
      ...
      <div class="targets">
        ${others.map(([id,s])=>`<div class="target-btn" data-act="${act}" data-val="${id}"><span class="avatar">${esc(mgInitials(s.name))}</span> ${esc(s.name)} ${id===seatId?'<span class="tag">(أنت)</span>':''}</div>`).join('')}
      </div>
      ${mgMafiaChatPanel()}
    </div>`;
  }
